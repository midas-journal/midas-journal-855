/*=========================================================================
  ...
  Module:    $RCSfile: itkBSplineDeformableTransformOpt.txx,v $
  Language:  C++
  ...
  if ( inside && m_SplineOrderOdd )
    ...
    for( unsigned int j = 0; j < SpaceDimension; j++ )
      ...
      if ( ( index[j] >= static_cast<ValueType>( m_ValidRegionLast[j] ) ) ||
            ( index[j] <= static_cast<ValueType>( m_ValidRegion.GetIndex()[j] ) )   
         )
        { 
        inside = false;
        break;
   ...
   for (unsigned int dim=0; dim<SpaceDimension; dim++)
      ...
      for (unsigned int counter=0; counter<indexes.Size(); counter++) 
        ...
        if (param_index < this->GetNumberOfParameters())
          {	 
          jacobianMatrix[dir][dim] += 1. / m_GridSpacing[dir] *  
      ...
      // The transformation being defined as T(x) = u(x) + x, we need
      // to add 1. on the diagonal of the displacement jacobian to get
