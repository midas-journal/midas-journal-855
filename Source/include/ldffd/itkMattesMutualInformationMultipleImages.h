/*=========================================================================
  ...
  Module:    $RCSfile: itkMattesMutualInformationMultipleImages.h,v $
  Language:  C++
...
=========================================================================*/
#ifndef __itkMattesMutualInformationMultipleImages_h
#define __itkMattesMutualInformationMultipleImages_h
...
/** \class MattesMutualInformationMultipleImages
 * \brief Computes the mutual information between two images to be 
 * registered using the method of Mattes et al.
 *
 * MattesMutualInformationMultipleImages computes the mutual 
...
class ITK_EXPORT MattesMutualInformationMultipleImages :
    public ImageToImageMetric< TFixedImage, TMovingImage >
{
public:

  /** Standard class typedefs. */
  typedef MattesMutualInformationMultipleImages  Self;
  ...
  itkTypeMacro(MattesMutualInformationMultipleImages, ImageToImageMetric);
...
protected:
  ...
  virtual ~MattesMutualInformationMultipleImages() {};
...
private:
  ...
  MattesMutualInformationMultipleImages(const Self&); 
  //purposely not implemented
  ...
  virtual void ComputeImageDerivatives( const unsigned int imageIndex,
  ...
  virtual void ComputePDFDerivatives( unsigned int indexImage,
  ...
  /* Vector of moving images. */
  
  class MovingImageAndTime
  {
  public:
    MovingImageAndTime()
    {
    ...
    /* FIXME: Should this be a const pointer?? */ 
    MovingImageConstPointer image;
    unsigned int timePoint;
  };
  
  
  std::vector <MovingImageAndTime*> m_MovingImages;
  typedef std::vector<double> MovingImageValuesArrayType;
  
  void TransformPoint( 
    unsigned int sampleNumber, 
    const ParametersType& parameters,
    MovingImagePointArrayType& mappedPointArray,
    bool& sampleOk,
    MovingImageValuesArrayType& movingImageValue ) const;

  // Vector of interpolators
  std::vector<typename InterpolatorType::Pointer> m_Interpolators;

public:
  
  virtual void SetNumberOfMovingImages(unsigned int nomi){
    m_MovingImages.resize(nomi);
  }
  
  using Superclass::SetMovingImage;
  void SetMovingImage(unsigned int pos, MovingImageType* im, unsigned int time){
    
    if (pos < m_MovingImages.size())
      {
      MovingImageAndTime* imt = new MovingImageAndTime;
      imt->image = im;
      imt->timePoint = time;
      std::cout << "inserting image at position " << pos <<", time " << time<<std::endl;
      m_MovingImages[pos] = imt;
      }
    else
      {
      itkExceptionMacro(<< "Incorrect position when inserting moving image in vector" << std::endl);
      }
  }


  /** BSpline interpolators. */
  std::vector<typename BSplineInterpolatorType::Pointer> m_BSplineInterpolators;
  
  /** Pointer to central difference calculators. */
  std::vector<typename DerivativeFunctionType::Pointer> m_DerivativeCalculators;
...
#include "itkMattesMutualInformationMultipleImages.txx"
#endif
